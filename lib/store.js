...
function inTurn(path, work) {
  const previous = queues.get(path) ?? Promise.resolve();
  const running = previous.then(
    () => withFileLock(path, work),
    () => withFileLock(path, work),
  );
  queues.set(
    path,
...
export class Store {
  ...
  async setCategories(data) {
    return inTurn(this.categoriesFile, () => this.writeCategories(data));
  }
  ...
  async setCodings(interview, codings) {
    return inTurn(this.codingFile(interview), () => this.writeCodings(interview, codings));
  }
