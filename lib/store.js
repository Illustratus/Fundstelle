...
import { seededPillars } from "./roles.js";
...
export class Store {
  constructor({
    ...
    this.rolesFile = join(dataDirectory, "roles.json");
  ...
  /* Role profiles -----------------------------------------------------------
     The other result of the first phase, and the one the tool does not make.
     A profile is written while reading a department's citations, so it is prose
     and it lives in a file of the study's own. What the tool adds is the join
     to the material: it resolves every locator to the turn it names, and from
     that follows what the prose cannot say — how much evidence a pillar rests
     on, and whose voice each piece of it is.
     ...
     and not form work; a text field would only invite it to be done badly. The
     file is edited where it is written and read back here. */

  async roles(language = this.seedLanguage) {
    const data = await read(this.rolesFile, null);
    if (data) {
      return {
        version: data.version ?? VERSION,
        pillars: Array.isArray(data.pillars) && data.pillars.length
          ? data.pillars.map(({ id, name }) => ({ id: String(id), name: String(name ?? id) }))
          : seededPillars(language),
        roles: Array.isArray(data.roles) ? data.roles : [],
      };
    }
    const fresh = { version: VERSION, pillars: seededPillars(language), roles: [] };
    await write(this.rolesFile, fresh);
    return fresh;
  }
