...
function ruleList(value) {
  if (!Array.isArray(value)) {
    throw Object.assign(new Error("Kodierregeln müssen eine Liste sein"), { status: 400 });
  }
  return value
...
function subordinate(categories, category, parentId) {
  if (category.origin === "deductive") {
    throw Object.assign(
      new Error("Deduktive Kategorien stehen vor der Erhebung fest und bleiben, wo sie sind"),
      { status: 409 },
    );
  }
  if (parentId === category.id) {
    throw Object.assign(new Error("Eine Kategorie lässt sich nicht sich selbst unterordnen"), {
      status: 400,
    });
  }
  const parent = parentId ? categories.find((c) => c.id === parentId) : null;
  if (parentId && !parent) {
    throw Object.assign(new Error("Unbekannte Kategorie"), { status: 404 });
  }
  ...
  if (parent?.parent) {
    throw Object.assign(
      new Error("Das Kategoriensystem bleibt zweistufig: unter einer Unterkategorie ist kein Platz"),
      { status: 409 },
    );
  }
  if (parent && categories.some((c) => c.parent === category.id)) {
    throw Object.assign(
      new Error("Diese Kategorie hat selbst Unterkategorien und kann nicht untergeordnet werden"),
      { status: 409 },
    );
  }
...
export class Store {
  constructor({ toolRoot, transcriptRoot, categoriesFile = null, startSystemFile = null }) {
  ...
  async categories() {
    ...
    const fresh = { version: VERSION, ...(await this.freshSystem()) };
  ...
  async freshSystem() {
    if (!this.startSystemFile) {
      return { propositions: PROPOSITIONS, categories: startSystem() };
    }
    let data;
    try {
      ...
      throw Object.assign(
        new Error(`Startsystem ${this.startSystemFile} ist nicht lesbar: ${error.message}`),
        { status: 500 },
      );
    }
    ...
    if (!Array.isArray(raw) || !raw.length) {
      throw Object.assign(
        new Error("Das Startsystem braucht eine nicht leere Liste „categories“"),
        { status: 500 },
      );
    }
    for (const category of raw) {
      if (!category.id || !category.name || !category.definition) {
        throw Object.assign(
          new Error(
            `Jede Startkategorie braucht id, name und definition (fehlt bei „${category.id ?? category.name ?? "?"}“)`,
          ),
          { status: 500 },
        );
      }
    ...
    const propositions = Object.fromEntries(
      Object.entries(data.propositions ?? data.propositionen ?? {}).map(([id, value]) => [
        id,
        normalizeProposition(value),
      ]),
    );
    return { propositions, categories: startSystem(raw) };
  }
  ...
  async addCategory({ name, definition, parent }) {
    ...
    if (data.categories.some((c) => c.id === id)) {
      throw Object.assign(new Error("Kategorie existiert bereits"), { status: 409 });
    }
  ...
  async updateCategory(id, fields) {
    ...
    if (!category) throw Object.assign(new Error("Unbekannte Kategorie"), { status: 404 });

    if ("definition" in fields) {
      const next = String(fields.definition ?? "").trim();
      if (!next) {
        throw Object.assign(new Error("Eine Kategorie ohne Definition ist nicht kodierbar"), {
          status: 400,
        });
      }
  ...
  async checkMerge(sourceId, targetId) {
    ...
    if (sourceId === targetId) {
      throw Object.assign(
        new Error("Eine Kategorie lässt sich nicht mit sich selbst verschmelzen"),
        { status: 400 },
      );
    }
    if (source.origin === "deductive") {
      throw Object.assign(
        new Error("Deduktive Kategorien sind vor der Erhebung festgelegt und bleiben bestehen"),
        { status: 409 },
      );
    }
  ...
  async removeCategory(id, used) {
    ...
    if (used > 0) {
      throw Object.assign(
        new Error(`Kategorie trägt ${used} Kodierungen und kann nicht entfallen`),
        { status: 409 },
      );
    }
  ...
  async addCoding(interview, entry) {
    ...
    if (conflict) {
      throw Object.assign(
        new Error("Die Auswahl überschneidet eine bestehende Kodiereinheit"),
        { status: 409, conflict },
      );
    }
  ...
  async updateCoding(interview, id, fields) {
    ...
    if (!coding) throw Object.assign(new Error("Unbekannte Kodierung"), { status: 404 });
  ...
  async removeCoding(interview, id) {
    ...
    if (remaining.length === data.codings.length) {
      throw Object.assign(new Error("Unbekannte Kodierung"), { status: 404 });
    }
  ...
  async addRequirement({ title, description = "", blockedOperations = [], moscow = null }) {
    if (!title?.trim()) {
      throw Object.assign(new Error("Die Anforderung braucht einen Titel"), { status: 400 });
    }
  ...
  async updateRequirement(id, fields) {
    ...
    if (!requirement) throw Object.assign(new Error("Unbekannte Anforderung"), { status: 404 });
  ...
  async checkRequirementMerge(sourceId, targetId) {
    ...
    if (!source || !target) {
      throw Object.assign(new Error("Unbekannte Anforderung"), { status: 404 });
    }
    if (sourceId === targetId) {
      throw Object.assign(
        new Error("Eine Anforderung lässt sich nicht mit sich selbst verschmelzen"),
        { status: 400 },
      );
    }
    return { source, target };
