...
import { MOSCOW, OPERATIONS } from "./analysis.js";
  ...
  return {
    ...
    paths: {
      ...
      "/api/interviews/{id}": {
        ...
        patch: {
          tags: ["Transcripts"],
          operationId: "setInterviewMemo",
          summary: "Set the memo on an interview",
          description:
            "The note about the interview as a whole — the conditions, what was noticed while " +
            "coding it. It travels into the analysis document and the appendix.",
          parameters: [INTERVIEW_ID],
          requestBody: {
            required: true,
            ...json({ type: "object", properties: { memo: { type: "string" } } }),
          },
          responses: { 200: { description: "Set.", ...json({ type: "object", properties: { memo: { type: "string" } } }) } },
        },
      },
          ...
          requestBody: {
            ...
            ...json({
              ...
              properties: {
                ...
                blockedOperations: {
                  type: "array",
                  items: { type: "string", enum: OPERATIONS.map((one) => one.id) },
                },
              },
            }),
          },
    ...
    components: {
      schemas: {
        ...
        Propositions: {
          type: "object",
          description:
            "The theoretical propositions categories are anchored in. Colour in this tool " +
            "encodes the proposition and nothing else — never the rank of a number.",
          additionalProperties: {
            type: "object",
        ...
        Requirement: {
          ...
          properties: {
            ...
            blockedOperations: {
              type: "array",
              items: { type: "string", enum: OPERATIONS.map((one) => one.id) },
              description: "Which operations its absence blocks — the second axis of the field.",
            },
            created: { type: "string", format: "date-time" },
            citations: {
              type: "array",
