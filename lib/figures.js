...
/** Which view a figure belongs to. */
export const viewOf = (name) => FIGURES[name]?.view ?? null;

/**
 * Which bodies of data a figure is drawn from — usually its own view's, and for
 * one of them both. Asked rather than assumed, so that fetching the catalog to
 * answer a question about the saturation curve stays work nobody does.
 */
export const needsOf = (name) =>
  FIGURES[name]?.needs ?? (FIGURES[name]?.view ? [FIGURES[name].view] : []);
 ...
 * a short list is cheaper to read than the documentation. A name that exists
 ...
 */
export function drawFigure(name, { analysis, catalog, roles, language, theme }) {
  if (!knows(name)) {
  ...
  const spec = FIGURES[name].draw({ analysis, catalog, roles, t });
  // Its own condition, not a shared "nothing to draw": which one is
...
/**
 * What the figures are, for whoever has not read the documentation.
 *
