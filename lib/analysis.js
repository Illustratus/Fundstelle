...
/**
 * What an operation is called, wherever it comes from.
 *
 * A study's own operations carry the name their author gave them; the three
 * this tool seeds carry a message key, so that a fresh study reads in the
 * language it was set up in. An identifier that is neither is written as it
 * stands rather than swallowed — a catalog that silently omits what a
 * requirement blocks is worse than one that shows an identifier.
 */
export function operationName(operation, t) {
  if (!operation) return null;
  return operation.name ?? (operation.key ? t(operation.key) : operation.id);
}

export function catalogMarkdown(rows, language, operations = OPERATIONS) {
  const t = translator(language);
  const byId = new Map(operations.map((one) => [one.id, one]));
  const parts = [
    `# ${t("catalogTitle")}`,
    "",
  ...
  for (const row of rows) {
    ...
    const blocked =
      (row.blockedOperations ?? [])
        .map((id) => operationName(byId.get(id), t) ?? id)
        .join(", ") || "—";
    parts.push(
  ...
  for (const row of rows) {
    ...
    /* The definition, and not the working note. The catalog export is the
       document that goes into the written work, and the note beside the
       definition is the desk it was worked out on — which is exactly what must
       not travel. */
