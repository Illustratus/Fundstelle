...
/**
 * The units that still have a place in the transcript.
 *
 * The comparison rests on "did this coder use this category in this turn", and
 * a unit whose passage was edited away answers that about a place that is gone.
 * It has to fall out on both sides, which is why the caller anchor-checks the
 * second coding too: dropping only the first coder's would turn a passage
 * nobody can point to into a disagreement between two people who never
 * disagreed.
 */
const placed = (codings) => codings.filter((coding) => coding.state !== "lost");
...
export function compare(interviews, categories, coder) {
  ...
  for (const { transcript, codings, others } of interviews) {
    ...
    const first = usedIn(placed(codings));
    const other = usedIn(placed(second));
